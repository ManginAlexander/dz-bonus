(function(toExport){
    ...
    Game.prototype.start = function ()
    {
        ...
        this.canvas.on('mouse:move', function(e) {
            ...
            var distance = that.state.location.distanceTo(mousePoint) - that.circle.radius - that.distanceBetweenCircleAndSmallAngle ;
            ...
            if (distance >= 0) {
                var vector = mousePoint.subtractWith(that.state.location).getNormalizedPoint();
                var k= (that.circle.radius + that.distanceBetweenCircleAndSmallAngle + (heightArrow)/2 + distance/4);
                var angle = new Line2d({"finish": mousePoint, "start": that.state.location}).getAbsAngle();
                if (arrow === null ) {
